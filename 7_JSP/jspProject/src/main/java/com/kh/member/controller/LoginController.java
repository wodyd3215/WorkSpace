...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser == null) {
			...
			// 조회결과 있음 -> 로그인 성공
			// 로그인된 회원정보를 session영역에 저장(why? 모든 페이지에서 사용할 수 있도록하기 위해)
			// Servlet에서는 session영역에 접근하고자 한다면 우선 session객체를 얻어야한다.
			HttpSession session = request.getSession();
			session.setAttribute("loginUser", loginUser);
			
			// 1. 포워딩방식 -> 해당방식은 url이 변경되지 않는다. 
			//    -> 우리는 localhost:8888/jsp/라는 기존의 메인 url이 있다.
			//	  그런데 해당 방식으로 응답시 url은 http://localhost:8888/jsp/login.me가 나타난다.
