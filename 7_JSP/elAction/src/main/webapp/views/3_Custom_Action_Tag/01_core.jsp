...
<body>
	...
	<pre>
		* 변수 선언과 동시에 초기화(c:set var="변수명" value="값" [scope="저장객체"])
		...
		- 해당 변수를 어떤 scope에 setAttribute를 통해담아둘지 결정할 수 있다.
	...
	<pre>
		* 변수 삭제(c:remove var="제거하고싶은 변수명" [scope=""])
		...
		- scope 지정 생략시 모든 scope에서 해당 변수를 모두 찾아서 제거한다.
	...
	<c:out value="${result}" default="없음"/><br> <%-- el인 ${result}와의 차이점 default설정 차이 --%>
	...
	<h3>6. url쿼리스트림</h3>
	<pre>
		- url경로를 생성하고, 쿼리스트림을 정의해줄 수 있는 태그
