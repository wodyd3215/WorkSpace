...
public class LoggingAOP {
	...
	/*
	 * <시점>
	 * @Before : 대상 메서드 실행 전에 Advice(추가 기능)이 실행된다.
	 * 
	 * @After : 대상 메서드 실행 후에 Advice(추가 기능)이 실행된다.
	 * 
	 * @AfterReturning : 대상 메서드가 정상적으로 반환된 후에 Advice(추가 기능)가 실행된다.
	 * 
	 * @AfterThrowing : 대상 메서드가 예외를 던진 후에 Advice(추가 기능)가 실행된다.
	 * 
	 * @Around : 대상 메서드를 감싸서 메서드 호출 전 후에 Advice(추가기능)를 실행할 수 있다.
	 */
	...
	public void before(JoinPoint joinPoint) {
		...
		log.info("Infomation		:	" + methodSignature);
		log.info("Method Name		:	" + method);
		log.info("Parameter			s:	" + Arrays.deepToString(args));
	}
	
	@AfterReturning(value="cut()", returning="obj") // 요청 후
